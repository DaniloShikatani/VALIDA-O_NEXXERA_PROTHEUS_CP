...
if arquivo_tesouraria and arquivo_nexxera:
    ...
    if st.button("Conciliar Relatórios"):
        with st.spinner("Mágica acontecendo... Lendo, padronizando e cruzando os dados..."):
            ...
            df_tesouraria = pd.read_csv(arquivo_tesouraria, sep=';', encoding='latin1')
            df_nexxera = pd.read_csv(arquivo_nexxera, sep=';', encoding='latin1')
            ...
            # Tabela da Nexxera
            df_nexxera.rename(columns={
                'Seu Número': 'ChaveTitulo',
                'Valor do Título': 'Valor'
            }, inplace=True)
            ...
            # --- A Conciliação (Merge) ---
            # Começa com a base da tesouraria e procura correspondência na Nexxera
            df_resultado = pd.merge(
                df_tesouraria,
                df_nexxera[['ChaveConciliacao', 'StatusVAN']], # Pega só as colunas chave da Nexxera
                on='ChaveConciliacao',
                how='left' # LEFT JOIN: mantém tudo da Tesouraria e traz o que achar na Nexxera
            )
            ...
            # Salva os resultados na memória do app (session_state)
...
if 'df_conciliados' in st.session_state:
    ...
    with tab_conciliados:
        ...
        st.download_button(
            "📥 Baixar Lista de Conciliados (CSV)",
            converter_df_para_csv(df_conciliados),
            "conciliados.csv", "text/csv"
        )
    ...
    with tab_nao_encontrados:
        ...
        st.download_button(
            "📥 Baixar Lista de Pendentes (CSV)",
            converter_df_para_csv(df_nao_encontrados),
            "pendentes_conciliacao.csv", "text/csv"
        )
